...
function goToStep(step) {
  ...
  document.body.classList.toggle('reveal-step-2', step === 2);
  ...
  updateExplainer();
  (step === 1 ? revealHeading : step2Heading).focus();
}
...
function announcePosterior() {
  ...
  announceTimer = setTimeout(() => {
    ...
    // One shared live region for both steps; only the wording differs.
    posteriorAnnouncement.textContent = activeExplainerStep === 2
      ? `${base} Distance from prior mean ${calculateDistanceFromPriorMean(parameters.mu).toFixed(2)}.`
      : base;
  }, 350);
}
...
function closeReveal() {
  ...
  document.body.classList.remove('reveal-active', 'reveal-step-2');
